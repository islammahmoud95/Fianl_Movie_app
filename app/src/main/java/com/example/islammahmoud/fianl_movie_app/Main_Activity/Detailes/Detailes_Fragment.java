...
import com.example.islammahmoud.fianl_movie_app.Main_Activity.Detailes.Data.Utility;
import com.example.islammahmoud.fianl_movie_app.Main_Activity.Detailes.Review.Review_Data;
import com.example.islammahmoud.fianl_movie_app.Main_Activity.Detailes.Review.Review_adapter;
import com.example.islammahmoud.fianl_movie_app.Main_Activity.Detailes.Trailer.trailer_adapter;
import com.example.islammahmoud.fianl_movie_app.Main_Activity.Detailes.Trailer.TrailerData;
import com.example.islammahmoud.fianl_movie_app.Main_Activity.Mainactivity.MovieData;
 ...
 */
public class Detailes_Fragment extends Fragment {
    public static String boaster_url(int width,String name)
    {
        return "http://image.tmdb.org/t/p/w"+Integer.toString(width)+name;
    }
    public static final String TAG = Detailes_Fragment.class.getSimpleName();
    ...
    private trailer_adapter mTrailerAdapter;
    private Review_adapter mReviewAdapter;
    ...
    private TrailerData mTrailer;


    public Detailes_Fragment() {
                            ...
                            // if it is not in favorites
                                ...
                                new AsyncTask<Void, Void, Uri>() {
                                    ...
                                    protected void onPostExecute(Uri returnUri) {
                                        item.setIcon(R.mipmap.beforelike );
                                        if (mToast != null) {
                                            mToast.cancel();
                                        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (arguments != null) {
            mMovie = arguments.getParcelable(Detailes_Fragment.DETAIL_MOVIE);
        }

        View rootView = inflater.inflate(R.layout.fragment_detailes, container, false);
        ...
        mTrailerAdapter = new trailer_adapter(getActivity(), new ArrayList<TrailerData>());
        mTrailersView.setAdapter(mTrailerAdapter);



        mReviewAdapter = new Review_adapter(getActivity(), new ArrayList<Review_Data>());
        mReviewsView.setAdapter(mReviewAdapter);

        if (mMovie != null) {


            String boster_url = boaster_url(342,mMovie.getImage2())  ;

            Picasso.with(getContext()).load(boster_url).into(mImageView);
    ...
    public class FetchTrailersTask extends AsyncTask<String, Void, List<TrailerData>> {
        ...
        private List<TrailerData> getTrailersDataFromJson(String jsonStr) throws JSONException {
            ...
            for(int i = 0; i < trailerArray.length(); i++) {
                ...
                if (trailer.getString("site").contentEquals("YouTube")) {
                    TrailerData trailerModel = new TrailerData(trailer);
        ...
        @Override
        protected void onPostExecute(List<TrailerData> trailers) {
            if (trailers != null) {
                if (trailers.size() > 0) {
                    mTrailersCardview.setVisibility(View.VISIBLE);
                    if (mTrailerAdapter != null) {
                        mTrailerAdapter.clear();
                        for (TrailerData trailer : trailers) {
    ...
    public class FetchReviewsTask extends AsyncTask<String, Void, List<Review_Data>> {

        private final String LOG_TAG = FetchReviewsTask.class.getSimpleName();

        private List<Review_Data> getReviewsDataFromJson(String jsonStr) throws JSONException {
            JSONObject reviewJson = new JSONObject(jsonStr);
            JSONArray reviewArray = reviewJson.getJSONArray("results");

            List<Review_Data> results = new ArrayList<>();

            for(int i = 0; i < reviewArray.length(); i++) {
                JSONObject review = reviewArray.getJSONObject(i);
                results.add(new Review_Data(review));
            }
        ...
        @Override
        protected void onPostExecute(List<Review_Data> reviews) {
            if (reviews != null) {
                if (reviews.size() > 0) {
                    mReviewsCardview.setVisibility(View.VISIBLE);
                    if (mReviewAdapter != null) {
                        mReviewAdapter.clear();
                        for (Review_Data review : reviews) {
